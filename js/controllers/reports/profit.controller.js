(function () {
    ...
    ReportProfitController.$inject = ['$location', '$timeout', 'DataService'];

    function ReportProfitController($location, $timeout, DataService) {
        var vm = this;
        vm.loading = false;
        ...
        vm.submitChartProfit = function(form) {
            ...
            $timeout(function() {
                ...
                if(form.chart == 1) {
                    chart = DataService.getChartProfitTotal();
                } else if(form.chart == 2) {
                    chart = DataService.getChartProfitTruck(vm.from, vm.to);
                } else if(form.chart == 3) {
                    chart = DataService.getChartProfitDriver(vm.from, vm.to);
                }
